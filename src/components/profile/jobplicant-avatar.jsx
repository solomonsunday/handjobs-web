...
const JobplicantAvatar = ({
  ...
  const avatarContainer = (fullname) => {
    ...
    return (
      <div className="avatar-container profile-picture">
        <h4 className="avatar-title">{getInitials(fullname)}</h4>
      </div>
    );
  };

  const fullname = profileInfo.firstName
  ...
  return (
    <form>
      <div
        ...
        onClick={isProfile ? () => handleClick() : () => {}}
        onMouseLeave={() => setCameraMode(false)}
        onMouseEnter={() => setCameraMode(true)}
      >
        <span className="profilePic-container">
          {profileInfo.imageUrl && profileInfo.imageUrl.includes("https") ? (
            <img
              src={selectedFile ? preview : profileInfo.imageUrl}
              alt="User Image"
              width="130"
              height="130"
