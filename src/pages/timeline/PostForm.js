...
const CreatePostModal = ({ post, clearModalInput }) => {
  ...
  const [selectedViewerGroup, setSelectedViewerGroup] = useState({
    name: "Any one",
    code: "PBC",
  });
  const baseClassName = isEmojiOpen
    ? "TextEditor-actions-emoji-picker-timeline--open"
    : "TextEditor-actions-emoji-picker-timeline";
  const viewerGroups = [
  ...
  const _handleClickEvent = (e) => {
    const { target } = e;
    const parentIds = [
      "TextEditor-actions-emoji-button",
      "TextEditor-actions-emoji-picker",
    ];
  ...
  const inputChange = (e, inputName) => {
    const inputValue =
      inputName && inputName === "body" ? e.htmlValue : e.target.value;
  ...
  const renderHeader = () => {
    return (
      <span className="ql-formats text-editor-timeline">
        ...
        >
          <Emoji emoji={{ id: "slightly_smiling_face" }} size={20} />
        </span>
        <div id="TextEditor-actions-emoji-picker" className={baseClassName}>
          <Picker
            showPreview={false}
            showSkinTones={false}
              ...
              {image.preview && (
                <div className="timelineMediaItem-container">
                  <div className="dropdown font-weight-bold ml-2 portfolioItem-icons">
                    ...
                    >
                      <li className="dropdown-item" onClick={deleteMediaItem}>
