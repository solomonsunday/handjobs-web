...
const EditJobModal = ({ showModal, onHide, job }) => {
  const editingJob = useSelector((state) => state.job.editingJob);
  ...
  useEffect(() => {
    if (job) {
      ...
      setCompanyInfo({
        ...
        endDate: job.endDate,
        minYearOfExperience: job.minYearOfExperience,
        country: countries.find(c => c.name === job.country),
        state: states.find(s => s.name === job.state),
        minYearOfExperience: job.minYearOfExperience,
        hideCompanyName: job.hideCompanyName,
        startDate: job.startDate,
        endDate: job.endDate
      });
      setEditorHtml(job.description)

      setValue('companyName', job.companyName)
      setValue('industry', job.industry)
      setValue('website', job.jobUrl)
      setValue('jobTitle', job.title)
      setValue('description', job.description)
      setValue('jobLocation', job.location)
      setValue('minQualification', job.minQualification)
  ...
  const onSubmit = () => {
    ...
    console.log('job id', job.id, 'data', dataToPost)
    return dispatch(editJob(job.id, dataToPost))
  };
  ...
  >
    ...
    {job ? <div className="content-body">
