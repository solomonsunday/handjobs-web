  ...
  switch (action.type) {
    ...
    case LOAD_FREE_USERS:
      ...
      if (state?.loadingContact === "loadMore") {
        const idArray = Array.from(
          new Set([...state.freeUsers.ids, ...data.map(({ id }) => id)])
        );
    ...
    case LOAD_CONTACTS:
      ...
      if (state?.loadingContact === "loadMoreContacts") {
        const idArray = Array.from(
          new Set([...state.contacts.ids, ...contactArray.map(({ id }) => id)])
        );
        uniqueContactIds.push(...idArray);
      } else {
        const idArray = Array.from(
          new Set([...contactArray.map(({ id }) => id)])
        );
        uniqueContactIds.push(...idArray);
      }
    ...
    case LOAD_PENDING_REQUESTS:
      ...
      if (state?.loadingContact === "loadMorePending") {
        ...
        const idArray = Array.from(
          new Set([...pendingArray.map(({ id }) => id)])
        );
        uniquePendingIds.push(...idArray);
      }
    ...
    case REMOVE_PENDING:
      ...
      updatedPendingArray.splice(
        state.pendingRequests.ids.indexOf(pendingId),
        1
      );
      return {
        ...state,
